...
async def get_unified_documents(
    ...
    if type_filter in [DocumentTypeFilter.ALL, DocumentTypeFilter.DOCS]:
        ...
        docs_service = DocsDocumentService(db)
        try:
            docs = await docs_service.list_documents(
                tenant_id=tenant_id,
                owner_id=owner_id,
                search=search,
            ...
            for doc in docs:
                if starred_only and not getattr(doc, 'is_starred', False):
                    continue
                items.append(UnifiedDocumentResponse(
                    ...
                    owner_id=str(doc.created_by) if hasattr(doc, 'created_by') else str(owner_id),
                    is_starred=getattr(doc, 'is_starred', False),
                    extra={"document_type": getattr(doc, 'document_type', 'GENERAL')}
                ))
    ...
    if type_filter in [DocumentTypeFilter.ALL, DocumentTypeFilter.SHEETS]:
        ...
        if shared_only:
            query = query.where(Spreadsheet.created_by != owner_id)  # Documents not owned by current user
    ...
    if type_filter in [DocumentTypeFilter.ALL, DocumentTypeFilter.SLIDES]:
        ...
        if shared_only:
            query = query.where(Presentation.created_by != owner_id)
    ...
    if type_filter in [DocumentTypeFilter.ALL, DocumentTypeFilter.VIDEOS]:
        ...
        try:
            ...
            if shared_only:
                query = query.where(Video.created_by != owner_id)
    ...
    if type_filter in [DocumentTypeFilter.ALL, DocumentTypeFilter.FORMS]:
        ...
        if shared_only:
            query = query.where(Form.created_by != owner_id)
...
async def toggle_star(
    ...
    if doc_type == "docs":
        docs_service = DocsDocumentService(db)
        doc = await docs_service.get_document(doc_id, tenant_id)
        if not doc:
...
async def get_productivity_stats(
    ...
    try:
        docs_service = DocsDocumentService(db)
        docs = await docs_service.list_documents(tenant_id=tenant_id, owner_id=owner_id, limit=1000)
        stats["docs"] = len(docs)
    except:
        stats["docs"] = 0
